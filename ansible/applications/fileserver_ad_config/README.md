...
them together. It also creates and updates the domain-local security groups named by the file
share declaration.
...
Managed principals follow `DOMAIN\DL-FS-Share[-Folder...]-RO|RW`; share and folder segments must
match the compact file-side declaration. `RO` pairs with `ReadAndExecute`, and `RW` pairs with
`Modify`. Explicitly unmanaged non-group principals are exempt. The directory role also refuses
caller-written protected principals or fixed access-rule fields, matching the file-server role's
compact-declaration boundary.

Only group existence, OU, domain-local security scope, and description are converged. Membership
