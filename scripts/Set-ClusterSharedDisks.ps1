...
<#
    ...
    .DESCRIPTION
        ...
        then adopts, owner-scopes, starts, reacquires, and verifies it.
...
$ConvertToSafeDiskState = {
  ...
  [PSCustomObject]@{
    ...
    disk_id_guid       = $State.disk_id_guid
    observed_unique_id = $State.observed_unique_id
    state              = $(If ($Null -eq $State.resource) { $Null } Else { [System.String]$State.resource.State })
    possible_owners    = @($State.possible_owners)
  }
}
...
$GetDeclaredDiskState = {
  ...
  $Resources = @(Get-ClusterResource -Cluster $Cluster | Where-Object -FilterScript { [System.String]$PSItem.ResourceType -eq 'Physical Disk' })
  $ResourceMatches = @()
  $MatchDetails = @{}
  ...
  If ($Null -ne $Resource) {
    ...
    $PossibleOwners = @(Get-ClusterOwnerNode -InputObject $Resource | ForEach-Object -Process { [System.String]$PSItem.Name })
  }
  [PSCustomObject]@{
    function           = $Function
    volume_id          = $VolumeId
...
$ClusterNodes = @(Get-ClusterNode -Cluster $ClusterName | ForEach-Object -Process { ([System.String]$PSItem.Name).ToLowerInvariant() })
...
$Before = & $GetDeclaredDiskState -Cluster $ClusterName -Function ([System.String]$Disk.Function) -VolumeId $VolumeId -Owners $Owners
...
If ($Null -eq $Before.resource) {
  ...
  If ([System.String]$Before.resource.State -ne 'Online') { $Actions.Add('start_disk') }
}
...
If ($Actions.Count -eq 0 -or $Ansible.CheckMode) {
  ...
  If ($Actions.Contains('adopt_disk')) {
    $Added = @(Add-ClusterDisk -InputObject $Before.local_disk -Cluster $ClusterName)
  ...
  If ($Null -eq $After.resource -or [System.String]::IsNullOrWhiteSpace([System.String]$After.disk_id_guid) -or
    -not $ObservedToken.Contains($Before.expected_serial) -or [System.String]$After.resource.State -ne 'Online' -or
...
$Result = [PSCustomObject]@{
  ...
  msg        = $(If ($Actions.Count -eq 0) { 'Shared disk already matches.' } ElseIf ($Ansible.CheckMode) { 'Check mode: shared disk would be converged.' } Else { 'Shared disk converged.' })
}
#endregion --- [ Main ] ---------------------------------------------------------------------- #
