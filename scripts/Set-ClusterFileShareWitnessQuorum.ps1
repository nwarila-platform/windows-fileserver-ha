...
<#
    .SYNOPSIS
        Converges a failover cluster on Majority quorum with an online file-share witness.
    .DESCRIPTION
        Resolves exactly one local cluster, compares its quorum type, quorum resource, state, and
        SharePath to the declared witness UNC, applies Set-ClusterQuorum when drift exists, and
        verifies readback.
...
$BeforeExact = (
  $BeforeQuorumType -eq 'Majority' -and
...
If ($Actions.Count -eq 0 -or $Ansible.CheckMode) {
  ...
  $AfterExact = (
    $AfterQuorumType -eq 'Majority' -and
  ...
  If (-not $After.exact) {
    Throw ('Quorum readback failed for {0} with witness {1}: observed QuorumType={2}, ResourceType={3}, State={4}, SharePath={5}.' -f @(
        $ClusterName
        $WitnessPath
        $After.quorum_type
        $After.resource_type
