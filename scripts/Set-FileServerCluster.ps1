...
Try {
  ...
  If ($Null -eq $FindPrestageComputerObject) {
    $FindPrestageComputerObject = {
      ...
      $SearchResults = $Null
      Try {
        # Path-only ADSI binds use the current process token; the role runs this script as the
        # cluster formation account, so the password parameter is not reused as an LDAP credential.
        ...
        $SearchRoot = [System.DirectoryServices.DirectoryEntry]::new(('LDAP://{0}' -f $DefaultNamingContext))
        ...
        $SearchResults[0].GetDirectoryEntry()
      } Finally {
  ...
  $SetPrestageComputerDisabled = {
    ...
    $Computer = $Null
    Try {
      $Computer = & $FindPrestageComputerObject -Name $Name
