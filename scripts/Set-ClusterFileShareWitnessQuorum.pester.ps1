...
BeforeAll {
  ...
  Function Set-ClusterQuorum {
    ...
    If (-not $global:FsQuorumFrozen) {
      $global:FsQuorumType = 'NodeAndFileShareMajority'
...
Describe 'Set-ClusterFileShareWitnessQuorum' {
  ...
  It 'returns exact standalone state without writes' {
    ...
    $Result.after.quorum_type | Should -Be 'NodeAndFileShareMajority'
  ...
  It 'converts Node Majority to the declared file-share witness' {
    $global:FsQuorumType = 'NodeMajority'
  ...
  It 'does not infer quorum type from an otherwise exact file-share witness' {
    $global:FsQuorumType = 'NodeMajority'

    $Result = & $script:ScriptPath @script:Common | ConvertFrom-Json

    $Result.before.exact | Should -BeFalse
    $Result.before.quorum_type | Should -Be 'NodeMajority'
    $Result.actions | Should -Be @('set_file_share_witness')
    $global:FsQuorumWrites | Should -HaveCount 1
  ...
  It 'fails when Set-ClusterQuorum does not survive readback' {
    $global:FsQuorumSharePath = '\\old-host\old$'
    $global:FsQuorumFrozen = $True

    { & $script:ScriptPath @script:Common } | Should -Throw '*Quorum readback failed*'
  }
