...
BeforeAll {
  ...
  $script:Common = @{
    Principal = 'TCN\North File Users'
    Description = 'Grants access to the North site folder'
    Path = 'OU=Windows HA FileServer,OU=Domain Servers,DC=tcn,DC=example,DC=com'
  }
  ...
  Function New-FakeGroup {
    Param (
      [System.String]$Name = 'North File Users',
      [System.String]$Description = 'Grants access to the North site folder',
      [System.String]$Path = 'OU=Windows HA FileServer,OU=Domain Servers,DC=tcn,DC=example,DC=com',
      [System.String]$Scope = 'DomainLocal',
...
Describe 'Set-FileShareAccessGroup' {
  ...
  It 'creates an absent domain-local security group and freshly verifies it' {
    ...
    $global:FsAccessGroupWrites[0].Name | Should -Be 'North File Users'
    $global:FsAccessGroupWrites[0].GroupScope | Should -Be 'DomainLocal'
  ...
  It 'converges name path description scope and category without membership arguments' {
    $global:FsAccessGroup = New-FakeGroup -Name 'Old Name' -Description 'old' `
      -Path 'OU=Old,DC=tcn,DC=example,DC=com' -Scope 'Global' -Category 'Distribution'
    $global:FsAccessGroup.sAMAccountName = 'North File Users'
    ...
    $Result.after.scope | Should -Be 'DomainLocal'
