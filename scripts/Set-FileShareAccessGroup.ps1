...
<#
    ...
    .DESCRIPTION
        Creates or updates one domain-local security group without changing membership or deleting
...
If ($BeforeMatches.Count -eq 0) {
  ...
  If ([System.String]$Before.sAMAccountName -cne $GroupName -or
    ...
    [System.String]$Before.GroupScope -ne 'DomainLocal' -or
...
If ($Actions.Count -gt 0 -and -not $Ansible.CheckMode) {
  If ($Null -eq $Before) {
    New-ADGroup -Name $GroupName -SamAccountName $GroupName -Description $Description `
      -GroupCategory Security -GroupScope DomainLocal -Path $Path -ErrorAction Stop
  } Else {
    ...
    If ($Actions.Contains('set_group_properties')) {
      Set-ADGroup -Identity $Identity -Description $Description -SamAccountName $GroupName `
        -GroupCategory Security -GroupScope DomainLocal -ErrorAction Stop
    }
  }
}
...
If (-not $Ansible.CheckMode) {
  ...
  If ([System.String]$After.GroupScope -ne 'DomainLocal') {
    $Disagreements.Add(('scope expected DomainLocal but was {0}' -f $After.GroupScope))
  }
